(function(exports){
  ...
  var Grid = function(columns, rows, engine) {
  ...
  Grid.prototype.entranceBlock = function(x, y) {
    ...
    var block = createGridNode(this.engine, place.x, place.y, this.blockSize, { fillStyle: "red"}, true )
  ...
  Grid.prototype.exitBlock = function(x, y) {
    var place = grid2Pix(x, y, this.blockSize)
    var block = createGridNode(this.engine, place.x, place.y, this.blockSize, { fillStyle: "green"}, true)
    this.setNode(x, y, block)
    this.exit = place
  }
  ...
  Grid.prototype.generateBlock = function (x, y, sizeX, sizeY) {
    for (var j = 0; j < sizeY; j++) {
      for (var i = 0; i < sizeX; i++) {
        ...
        var block = createGridNode(this.engine, place.x, place.y, this.blockSize);
  ...
  function createGridNode(engine, x, y, size, style = {}, sensor = false ) {
